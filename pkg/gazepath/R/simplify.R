  ...
  simple <- data.frame(class$values, class$lengths, 
    ...
    if(length(which(class$values == 'f')) > 0){
      ...
      for(i in 1:dim(simple)[1]){
        ...
        POGvar <- c(POGvar, sqrt(mean(as.matrix(dist(cbind(c(mean_x[length(mean_x)], x[simple[i,3] : simple[i,4]]), c(mean_y[length(mean_y)], y[simple[i,3] : simple[i,4]]))))[-1,1])))
      }
      ## Calculate saccade amplitude and transform POGsd from pixels to degrees of visual angle
      ...
      simple <- data.frame(class$values, class$lengths * (1000/Hz), 
                           ...
                           cumsum(class$lengths * (1000/Hz)), x_start, y_start, x_end, y_end, mean_x, mean_y, POGsdSacAmp)
